...
function buildSystemPrompt(state) {
  ...
  return [
    ...
    "",
    "NOTE: PRESERVE RICHNESS. List the items separated by + or , — the user wants",
    "  to remember WHAT they bought, not just a category. Examples:",
    "    Receipt items: Latte, Croissant, Sparkling Water → note: \"latte + croissant + sparkling water\"",
    "    Grocery receipt: Milk, Bread, Eggs, Tomatoes → note: \"milk + bread + eggs + tomatoes\"",
    "    Restaurant: Pho Bo, Spring Rolls, Beer → note: \"pho bo + spring rolls + beer\"",
    "  If the receipt has many items (>6), keep the first 5-6 distinguishing ones,",
    "    add \"…\" or \"+ N more\". Don't truncate to a single word.",
    "  Lowercase ok — feels natural in chat. Use the user's likely phrasing.",
    "",
    "TAGS (optional, helpful for richer search later):",
    "  - \"travel\" if receipt is from a different country than user's base",
    "  - \"work\" if it looks like a business expense (catering, multiple covers)",
    "  - vendor type chips (\"cafe\", \"market\", \"restaurant\") only when ambiguous from category",
    "",
    "CONTEXT (optional):",
    "  If location is clear on the receipt (city in address line) and it's NOT the",
    "  user's home country, include a context like \"vietnam trip\" / \"london trip\".",
    "  When unsure, OMIT — never guess.",
    "",
    ...
    "",
    "EXAMPLES (notice the rich notes — items preserved, not collapsed):",
    "- Vietnamese receipt with \"100.000\" total, vendor \"PHO 24\", items \"Pho Bo, Spring Rolls, Beer\":",
    '  → {"ok":true, "intent":{"kind":"record_spend","params":{"amountCents":0,"originalAmount":100000,"originalCurrency":"VND","vendor":"Pho 24","category":"restaurant","note":"pho bo + spring rolls + beer","tags":["travel"],"context":"vietnam trip"}}}',
    "- US receipt with \"$24.50\" total, vendor \"STARBUCKS\", items \"Latte, Croissant, Water\":",
    '  → {"ok":true, "intent":{"kind":"record_spend","params":{"amountCents":0,"originalAmount":24.50,"originalCurrency":"USD","vendor":"Starbucks","category":"coffee","note":"latte + croissant + water"}}}',
    "- Receipt with many items (8+) like a grocery haul, vendor WHOLE FOODS, total $87.42:",
    '  → {"ok":true, "intent":{"kind":"record_spend","params":{"amountCents":0,"originalAmount":87.42,"originalCurrency":"USD","vendor":"Whole Foods","category":"groceries","note":"milk + bread + eggs + tomatoes + spinach + 6 more"}}}',
