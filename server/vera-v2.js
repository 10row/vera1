...
function buildSystemPrompt(state) {
  ...
  const P = [
    ...
    "","STATE:", snap, "",
    "TODAY:" + today(), "",
    "JOB: If not set up: need balance, income, payday. Ask missing only.",
    "Send setup ONLY with ALL THREE. After: ask saving rate, then bills.",
    "If set up: log spending, check free, manage finances.",
    "","ACTIONS:",
    "setup:balanceUSD,incomeUSD,savingRate(0-1),payday(YYYY-MM-DD),recurring",
    "transaction:amountUSD(+spent,-refund),description",
    "add_drain:name,amountUSD,intervalDays,nextDate|remove_drain:name",
    ...
    "set_saving_rate:rate(0-1)|set_savings:amountUSD|withdraw_savings:amountUSD,reason",
    "set_location:currency,symbol|reset|none",
    "","CURRENCY: Detect from language/context. Russian->RUB/₽. On first setup fire set_location.",
