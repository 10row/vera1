"use strict";
// tg-format-v3.js — V3 Telegram formatting
// No monospace blocks. Conversational. Hero number on every state change.
...
const S = {
  en: {
    ...
    skippedConfirm: "Skipped *NAME* — next date moved forward.",
    undone: "Done — last action undone.",
  ...
  ru: {
    ...
    undone: "Готово — последнее действие отменено.",
    freeToday: "Сегодня",
    days: "д",
    daysWord: "дн.",
    due: "к оплате",
...
// ── HERO LINE ─────────────────────────────────
// Appended after every state-changing AI response.
...
// ── MORNING BRIEFING ──────────────────────────
// Sent proactively. No monospace. Conversational.
function formatBriefing(pic, lang) {
  ...
  if (pic.freeCents < 0) {
    lines.push(lang === "ru" ? "Утро. Перерасход." : "Morning. You're over budget.");
  } else if ((pic.freeRemainingTodayCents || 0) < 500) {
    lines.push(lang === "ru" ? "Утро. Туго сегодня." : "Morning. It's tight today.");
  } else {
    lines.push(lang === "ru" ? "Доброе утро!" : "Good morning!");
  }
  ...
  // Pace context
  const pace = pic.dailyPaceFormatted || M(pic.dailyPaceCents || 0);
...
// ── ENVELOPE ALERT ────────────────────────────
// For proactive due-bill notifications
...
// ── KEYBOARD ──────────────────────────────────
// V3: two buttons only — review + dashboard
...
// Due envelope inline buttons (pay / skip)
function dueButtons(envelopeName, lang) {
  return {
    inline_keyboard: [[
      { text: t(lang, "paidBtn") + envelopeName, callback_data: "pay:" + envelopeName },
      { text: t(lang, "skipBtn"), callback_data: "skip:" + envelopeName },
    ]],
  };
}

// Receipt confirmation buttons
...
function formatReconciliation(pic, lang) {
  const sym = pic.currencySymbol || "$";
  const bal = v3.toMoney(pic.balanceCents, sym);
  return lang === "ru"
    ? "Быстрая проверка — на счёте всё ещё около " + bal + "?"
    : "Quick check — is your bank balance still around " + bal + "?";
}

module.exports = {
  S, t, detectLang, today, daysUntil,
  heroLine, formatBriefing, formatEnvelopeAlert,
  formatReconciliation,
  mainKeyboard, dueButtons, receiptButtons,
};
