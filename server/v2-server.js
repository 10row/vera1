...
async function getState(webToken) {
  const userId = await resolveUser(webToken);
  return { userId, state: await db.loadState(prisma, userId) };
}
...
app.post("/api/v2/action/:sessionId", async (req, res) => {
  ...
  try {
    ...
    res.json({ pic, state: sanitise(newState) });
  } catch (err) {
...
app.post("/api/v2/query/:sessionId", async (req, res) => {
  ...
  try {
    const { state } = await getState(req.params.sessionId);
