...
async function persist(userId, state) { await db.saveState(prisma, userId, state); }
...
app.get("/api/v2/picture/:sessionId", async (req, res) => {
  try {
    ...
    res.json({ pic: v2.computePicture(state), state: sanitise(state) });
  } catch (err) { console.error("Picture error:", err.message); res.status(500).json({ error: err.message }); }
});
...
app.post("/api/v2/action/:sessionId", async (req, res) => {
  ...
  try {
    ...
    const result = await db.withUserLock(userId, async () => {
      ...
      return { pic: v2.computePicture(newState), state: sanitise(newState) };
    });
    res.json(result);
  } catch (err) { console.error("Action error:", err.message); res.status(500).json({ error: err.message }); }
});
  ...
  try {
    ...
    const result = await db.withUserLock(userId, async () => {
      ...
      const response = await anthropic.messages.create({
        ...
        system: v2.buildSystemPrompt(state), messages: history,
      });
      ...
      for (const action of (parsed.actions || [])) { state = v2.applyAction(state, action); }
      const queryResults = {};
      for (const query of (parsed.queries || [])) { queryResults[query.type] = v2.runQuery(state, query); }
      await persist(userId, state);
      return { message: parsed.message, actions: parsed.actions, queries: parsed.queries, queryResults, pic: v2.computePicture(state), state: sanitise(state) };
    });
    res.json(result);
  } catch (err) { console.error("Chat error:", err.message); res.status(500).json({ error: "AI error: " + err.message }); }
});
...
app.post("/api/v2/query/:sessionId", async (req, res) => {
  ...
  try {
    const userId = await resolveUser(req.params.sessionId);
    const state = await db.loadState(prisma, userId);
    res.json({ result: v2.runQuery(state, query) });
  } catch (err) { console.error("Query error:", err.message); res.status(500).json({ error: err.message }); }
});
...
app.post("/api/v2/reset/:sessionId", async (req, res) => {
  try {
    ...
    await prisma.$transaction([
      ...
      prisma.user.update({ where: { id: userId }, data: { setup: false, balanceCents: 0, incomeCents: 0, savingsCents: 0, savingRateBps: 0, payday: null, cycleStart: null, localRate: 100, language: "en" } }),
    ]);
    res.json({ ok: true });
  } catch (err) { console.error("Reset error:", err.message); res.status(500).json({ error: err.message }); }
});
...
app.listen(PORT, async () => {
  ...
  if (!process.env.ANTHROPIC_API_KEY) console.log("  WARNING: No ANTHROPIC_API_KEY set");
  if (bot && process.env.BOT_TOKEN) {
    if (process.env.WEBHOOK_URL) {
      try {
        await bot.init();
