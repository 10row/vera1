...
function buildSystemPrompt(state) {
  ...
  const lines = [
    ...
    "13. FUNDING vs ADDING — \"put $700 toward Vietnam hotel\" / \"save 500 for X\" / \"add 200 to Y fund\" → fund_envelope on the EXISTING envelope (look in STATE.envelopes for a name match — even fuzzy, like \"Vietnam hotel\" matches \"vietnam hotel\"). Only emit add_envelope when the user clearly means a NEW envelope. If a similar-named envelope exists and the user is putting money toward it, fund it; never duplicate.",
    "14. NAME COLLISION — when the user mentions a category that's already in STATE.envelopes, do NOT emit add_envelope with the same or similar name. Either fund_envelope (if they're adding money), update_envelope (if they're changing fields), or pick a clearly different name (e.g. \"Vietnam trip\" vs an existing \"Vietnam hotel\").",
    "",
    ...
    "  Phase 1 — ASK BALANCE.",
    "    If user hasn't given a balance: TALK mode, ask warmly. \"Hey 👋 What's the rough balance in your main account right now?\"",
    "    If user gave a balance: DO mode, emit setup_account with just balanceCents. The engine fills in defaults (payday = today+30d, payFrequency = monthly). Your reply message asks about their payday: \"Got it. When's your next paycheck?\"",
